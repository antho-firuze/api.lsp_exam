	...
	*/
	...
	function __construct()
	{
		...
		if (!in_array($this->r_method, $this->r_method_allowed))
			$this->f->response(FALSE, ['message' => $this->f->_err_msg('err_request_method_unsupported')], 403);
		...
		if (in_array($this->r_method, ['POST','PUT','OPTIONS','PATCH','UNLOCK','LOCK'])) {
			...
			if (!in_array(gettype($request), ['object', 'array']))
				$this->f->response(FALSE, ['message' => $this->f->_err_msg('err_request_invalid')]);
	...
	private function is_valid_request($request)
	{
		if (!$request || is_string($request))
			return [FALSE, ['message' => $this->f->_err_msg('err_request_invalid')]];
	...
	private function is_valid_agent($request)
	{
		...
		if (!in_array($request->agent, $this->agent))
			return [FALSE, ['message' => $this->f->_err_msg('err_param_unsupported', "{agent: $request->agent}")]];	
	...
	private function is_valid_language($request)
	{
		if (isset($request->lang)) {
			if (! in_array($request->lang, array_keys($this->languages))) 
				return [FALSE, ['message' => $this->f->_err_msg('err_param_unsupported', "{lang: $request->lang}")]];	
	...
	private function requery_request($request)
	{
		if (is_object($request)){
			...
			if (count((array)$request) < 1)
				$this->f->response(FALSE, ['message' => $this->f->_err_msg('err_request_invalid'), 'id' => null]);
	...
	private function exec_method($request)
	{
		if (!isset($request->method))
			return $this->f->response(FALSE, ['message' => $this->f->_err_msg('err_param_required', 'method'), 'id' => (isset($request->id) ? $request->id : null)], FALSE, FALSE);
		...
		if (count($parseMethod) < 2)
			return $this->f->response(FALSE, ['message' => $this->f->_err_msg('err_method_unknown', $request->method), 'id' => (isset($request->id) ? $request->id : null)], FALSE, FALSE);
